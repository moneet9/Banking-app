...
const navItems = [
  ...
  { icon: Receipt, label: 'Passbook', path: '/passbook' },
        ...
        <div className="grid grid-cols-4 gap-2">
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                <item.icon 
                  className={`w-6 h-6 ${isActive ? 'text-[#0066FF]' : 'text-gray-500'}`}
                />
                <span className={`text-xs font-medium ${isActive ? 'text-[#0066FF]' : 'text-gray-500'}`}>
                  {item.label}
                </span>
              </button>
            );
